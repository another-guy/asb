...
| F01.3 | context use                 | `asb context use <name>` — set the active context in `~/.asb/config`                                                                   | complete |
...
| F11   | Message peek — subscription | `asb message peek <topic>/<sub> [--count]` — non-destructive inspection of subscription messages                                       | complete |
| F12   | Message peek — DLQ          | `asb message peek <target> --dlq [--count]` — inspect dead-letter sub-queue contents without acquiring a lock                          | complete |
